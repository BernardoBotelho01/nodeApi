...
//listar por id
router.get("/situacaoproduto/:id",async(req:Request, res:Response)=>{
    ...
    return
        ...
        res.status(500).json({
            messagem: "Erro ao buscar a situação do produto pelo ID informado!"
        });
        return
    }
});
...
//atualizar
router.put("/situacaoproduto/:id",verificarToken, async (req: Request, res: Response) => {
  try {
    const id = Number(req.params.id);
    ...
    return res.status(500).json({
      messagem: "Erro ao atualizar a situação do produto!",
    });
  }
});
//deletar
router.delete("/situacaoproduto/:id",verificarToken,async(req:Request, res:Response)=>{
   ...
   await productSituationRepository.remove(productSituation);
    ...
    res.status(200).json({
        ...
        res.status(500).json({
            messagem: "Erro ao remover a situação do produto!"
        });
        return
    }
});
